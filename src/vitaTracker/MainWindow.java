	...
	private class ReadFileIntoDatabase implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				// Verhindern, dass wÃ¤hrend des Imports BenutzermenÃ¼-Funktionen
				// aufgerufen werden kÃ¶nnen.
				//
				...
				while( scanner.hasNext() )
				{
					...
					if( readCounter % 10 == 0 )
						sbStaBarMainWin.setMessage(
							String.format("Es werden Datensätze eingelesen [%s]",
							NumberFormat.getInstance().format(readCounter)));
					
					// Methode 'split' mit maximaler Anzahl 
					// zurückzuliefernder Zeichenketten
					//
					split = zeile.split(";", 5);
			...
			// BenutzermenÃ¼ wieder aktivieren.
			//
			for( int i = 0; i < menuBar.getMenuCount(); i++ )
			...
			JOptionPane.showMessageDialog(
				null,
				String.format("Es wurden %s DatensÃ¤tze erfolgreich eingelesen",
